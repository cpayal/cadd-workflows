...
#warnings.filterwarnings("ignore", category=pcp.PubChemPyDeprecationWarning)
...
def write_pdbqt_from_df(df, mol_col="Mols", id_col="CID",
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            ####### Inserting flag to ensure poor ending of pdbqt without 'ENDMDL' endings:
            #pdbqt_file = ""
            #for i, ms in enumerate(setup_list):
            #    pdbqt_file = os.path.join(outdir, f"{lig_id}.pdbqt" if i == 0 else f"{lig_id}__alt{i+1}.pdbqt")
            #    pdbqt_str = writer.write_string(ms)
            ...
            #    pdbqt_str = pdbqt_str.rstrip()
            #    if not pdbqt_str.endswith("ENDMDL"):
            #        pdbqt_str += "\nENDMDL\n"
            #
            #    with open(pdbqt_file, "w") as fh:
            #        fh.write(pdbqt_str)
